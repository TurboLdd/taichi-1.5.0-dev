    ...
    if (from_order > to_order) {  // high-to-low relation
      if (stmt->is_size()) {
        ...
        if (from_type == mesh::MeshElementType::Cell) {
          if (stmt->to_type == mesh::MeshElementType::Face) {
            stmt->replace_with(Stmt::make<ConstStmt>(TypedConstant{6}));
          } else if (stmt->to_type == mesh::MeshElementType::Edge) {
            stmt->replace_with(Stmt::make<ConstStmt>(TypedConstant{12}));
          } else if (stmt->to_type == mesh::MeshElementType::Vertex) {
            stmt->replace_with(Stmt::make<ConstStmt>(TypedConstant{8}));
          }
